...
function cursorDiffHistoryFromUnifiedDiff(diff: string): string {
  if (/^\d+[+-]\|/m.test(diff)) {
    return diff;
  }
...
function normalizeFileDiffHistories(payload: any, now: number) {
    ...
    (history: any) => {
      ...
      return {
        ...
        diffHistoryTimestamps:
          history.diffHistoryTimestamps?.length === diffHistory.length
